...
|Building SAPK fails
|Deploying apps to a device or building SAPK fails. In the SICK App Designer Log output channel a message shows: `Failed to build msdd`. This is caused by a corrupted UIBuilder control-library.
|1. Close VS Code. +
...
|No error log in OUTPUT > SICK App Designer Build if `.msdd` build breaks.
...
|Update of Device connectivity causes loss of device list/configuration.
...
|Restart VS Code. +
Note: All VS Code instances with active App Designer must be restarted (shared device connectivity).
